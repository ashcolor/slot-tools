...
```typescript
interface Member {
  investMedals: number;  // 投資メダル枚数（貯メダルから使った枚数）
  investCash: number;    // 投資現金額
  collectMedals: number; // 回収メダル枚数（台から回収した総枚数）
  storedMedals: number;  // 貯メダル枚数（回収メダルのうち貯メダルに回す枚数）
}
```

### 計算フロー

#### Step 1: 投資メダルと貯メダルを相殺

```
medalDiff = storedMedals − investMedals
```

- `medalDiff >= 0` → 貯メダルが増えた（メダル資産が増加）
- `medalDiff < 0` → 貯メダルが減った（メダル資産が減少）
...
- 換金メダルは交換レートで現金化
- 貯メダル増加分 → 交換レートで評価（売ればその値段になる）
- 貯メダル減少分 → 貸出レートで評価（買い直すとその値段がかかる）
...
投資460枚, 回収460枚, 貯メダル460枚, 現金0円 (貸出20円, 交換20円)
...
投資460枚, 回収920枚, 貯メダル460枚, 現金0円 (貸出20円, 交換20円)
...
投資460枚, 回収460枚, 貯メダル0枚, 現金0円 (貸出20円, 交換18.18円)
